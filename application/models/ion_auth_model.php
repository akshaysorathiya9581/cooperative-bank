...
class Ion_auth_model extends CI_Model {
    ...
    public function create_group($group_name = FALSE, $group_description = '', $additional_data = array()) {
        ...
        $existing_group = $this->db->get_where($this->tables['groups'], array('name' => $group_name,'PIN'=>  current_user()->PIN))->num_rows();
        if ($existing_group !== 0) {
            $this->set_error('group_already_exists');
            return FALSE;
        }

        $data = array('name' => $group_name, 'description' => $group_description,'PIN'=>  current_user()->PIN);
    ...
    public function update_group($group_id = FALSE, $group_name = FALSE, $additional_data = array()) {
        ...
        if (!empty($group_name)) {
            ...
            // bail if the group name already exists
            $existing_group = $this->db->get_where($this->tables['groups'], array('name' => $group_name,'PIN'=>  current_user()->PIN))->row();
    ...
    function privilege_list($group_id) {
        ...
        foreach ($module as $key => $value) {
            ...
            $get_role = $this->db->get_where('role', array('Module_id' => $value->id))->result();
